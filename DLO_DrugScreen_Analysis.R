# Data Cleaning and Analysis For Drug Screen Disparities
...
#Load Data from NHAMCS files (downloaded from cdc.gov)
...
df <- read_dta("data-raw/ED2017-stata.dta") %>%
  select(VMONTH,VDAYR,ARRTIME,ARREMS,WAITTIME,AGE,SEX,RACEUN,ETHUN,
         ...
         CTCHEST,CTHEAD,CTOTHER,CTUNK,ADMIT,MED,MED1:MED30,GPMED1:GPMED30,TOXSCREN 
  ) %>% data.frame() %>% zap_labels()

df <- read_dta("data-raw/ED2018-stata.dta") %>%
  select(
    VMONTH, #Visit month (01-12)
    ...
    TOXSCREN #Urine Toxicology Screen  (1 = Yes, 0 = No)
  ) %>% data.frame() %>% zap_labels() %>% 
  rbind(df)

df2 <- read_dta("data-raw/ED2016-stata.dta") %>%
  select(
...
# Condense to the data being analyzed 
